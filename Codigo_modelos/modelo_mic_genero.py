...
micros_gen = [ microbiota_quanta_visit(6, visita).reset_index() for visita in [1,2,3]]
# concatenar cada df de cada visita en un solo df
todas_gen = pd.concat(micros_gen, axis=0)
todas_gen.rename(columns = {"visita":"etapas", "patient_id":"patient"}, inplace=True)

#print(todas_gen)

epsilones = pd.read_csv('e_genero.csv')
print(epsilones)
columnas = epsilones['Caracteristica'].unique()
lista = columnas.tolist()
lista.extend(['patient', 'etapas'])
print(lista)
mic_modelo = todas_gen[lista].copy()
print(mic_modelo)
...
train = procesar_mit(train)
test = procesar_mit(test)
